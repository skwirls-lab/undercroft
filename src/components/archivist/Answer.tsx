...
/**
 * The Archivist writes a small, predictable markdown: "##" headings, "-" bullets, numbered
 * lists, and **bold** for card names. This renders exactly that and nothing more, streaming
 * safely (a half-written line is just a paragraph until it finishes). Bold card names become
 * buttons when a handler is given, so a recommendation can be read before it is taken.
 */
export function Answer({ text, onCard, className }: { text: string; onCard?: (name: string) => void; className?: string }) {
...
type Block = { kind: 'heading'; text: string } | { kind: 'ul'; items: string[] } | { kind: 'ol'; items: string[] } | { kind: 'p'; text: string };
